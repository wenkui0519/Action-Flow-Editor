    ...
    () => ({
      ...
      history: {
        ...
        onApply: debounce((ctx, opt) => {
          ...
          // 数据变化都走到这里从change事件，所以一些节点操作，数据外发都在这里处理
          // 撤销回退时数据外发
          if (typeof config?.onchange === 'function') {
            const data = ctx.document.toJSON();
            config.onchange(data.nodes, opt);
          }
        }, 100),
      },
      /**
      ...
      materials: {
        components: {
          ...
          [FlowRendererKey.ADDER]: NodeAdder, // Node Add Button 添加节点按钮
          [FlowRendererKey.BRANCH_ADDER]: BranchAdder, // Branch Add Button 添加分支节点按钮
          [FlowRendererKey.DRAG_NODE]: DragNode, // Component in node dragging
        },
        renderDefaultNode: BaseNode, // node render
        renderTexts: {
          'loop-end-text': langs.loop_end_text || 'Loop End',
          'loop-traverse-text': langs.loop_traverse_text || 'Loop',
          'try-start-text': langs.try_start_text || 'Try Start',
          'try-end-text': langs.try_end_text || 'Try End',
          'catch-text': langs.catch_text || 'Catch Error',
        },
      },
      /**
       * Bind custom service
